...
namespace InitiateMod.NPCs.Bosses.Hellwurm
{
		...
		public override void CustomBehavior()
		{
			if (Main.netMode != 1)
			{
				...
				if (attackCounter <= 0 && Vector2.Distance(npc.Center, target.Center) < 200 && Collision.CanHit(npc.Center, 1, 1, target.Center, 1, 1))
				{
                    ...
                    int projectile = Projectile.NewProjectile(npc.Center, direction * 1, ProjectileID.GreekFire1, 50, 0, Main.myPlayer);
	...
	{
	Item.NewItem(npc.getRect(), mod.ItemType("HellwurmTrophy"));    
	}	
        ...
        {
            Item.NewItem(npc.getRect(), mod.ItemType("HellflameShadowspark"), 16);
	    Item.NewItem(npc.getRect(), ItemID.LivingFireBlock, 35);
        }
	...
	public abstract class Hellwurm : WormBoss1
	{
		...
		public override void Init()
		{
			minLength = 12;
			maxLength = 16;
			tailType = mod.NPCType<WormTail>();
			bodyType = mod.NPCType<WormBody>();
			headType = mod.NPCType<WormHead>();
			speed = 5.5f;
			turnSpeed = 0.045f;
		}
	}
	...
	public abstract class WormBoss1 : ModNPC
	{
		/* ai[0] = follower
		 * ai[1] = following
