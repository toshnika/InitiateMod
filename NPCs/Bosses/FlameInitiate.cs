...
{
    public class FlameInitiate : ModNPC
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Flame Initiate");
    ...
    public override void AI()
    {
        ...
        else if (AI_State == State_Fireball)
        {
            AI_Timer++;
            if (AI_Timer == 1)
            {
                Vector2 velocity = Vector2.Normalize(Main.player[npc.target].Center - npc.Center);
                Projectile.NewProjectile(npc.Center.X, npc.Center.Y, velocity.X, velocity.Y, Type: ProjectileID.CultistBossFireBall,Damage: 135, KnockBack: 4f);
                   Projectile.NewProjectile(npc.Center.X, npc.Center.Y, velocity.X, velocity.Y, Type: ProjectileID.CultistBossFireBall , Damage: 150, KnockBack: 7f);
            }
            else if (AI_Timer > 40)
            {
        ...
        else if (AI_State == State_Minions)
        {
            ...
            if (AI_Timer == 1)
            {
                ...
                NPC.NewNPC((int)npc.Center.X, (int)npc.position.Y + npc.height, NPCID.MeteorHead);
                NPC.NewNPC((int)npc.Center.X, (int)npc.position.Y + npc.height, NPCID.MeteorHead);
        ...
        else if (AI_State == State_Minions2)
        {
            ...
            if (AI_Timer == 1)
            {
                ...
                NPC.NewNPC((int)npc.Center.X, (int)npc.position.Y + npc.height, NPCID.RedDevil);
	...
	{
	Item.NewItem(npc.getRect(), mod.ItemType("FlameInitiateTrophy"));    
	}	
        ...
        if (Main.expertMode)
        {
            Item.NewItem(npc.getRect(), mod.ItemType("FlameBag"));
        }
        else
        {
            Item.NewItem(npc.getRect(), mod.ItemType("FlameEssence"), 10);
        }
