...
namespace InitiateMod.NPCs.Bosses.DestroyerOfWorlds
{
	class DOWHead : DOW
	{
	...
	{
	Item.NewItem(npc.getRect(), mod.ItemType("DestroyerOfWorldsTrophy"));    
	}	
        ...
        if (Main.expertMode)
        {
            Item.NewItem(npc.getRect(), mod.ItemType("DOWBag"));
        }
        else
        {
            Item.NewItem(npc.getRect(), mod.ItemType("DestroyerScale"), 25);
        }
			...
			public override void CustomBehavior()
		{
			if (Main.netMode != 1)
			{
				...
				if (attackCounter <= 0 && Vector2.Distance(npc.Center, target.Center) < 200 && Collision.CanHit(npc.Center, 1, 1, target.Center, 1, 1))
				{
					...
					int projectile = Projectile.NewProjectile(npc.Center, direction * 1, ProjectileID.PhantasmalBolt, 76, 0, Main.myPlayer);
	...
	public abstract class DOW : Worm
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Destroyer Of Worlds");
		}
		...
		public override void Init()
		{
			minLength = 50;
			maxLength = 50;
			tailType = mod.NPCType<DOWTail>();
			bodyType = mod.NPCType<DOWBody>();
			headType = mod.NPCType<DOWHead>();
			speed = 5.5f;
			turnSpeed = 0.045f;
		}
	}
