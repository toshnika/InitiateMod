...
{    
    public class FrozenWormHead : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            npc.defense = 12;         //this is the npc defense
        ...
        public override bool PreAI()
        {
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    int randomWormLength = Main.rand.Next(10, 15);
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("FrozenWormBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("FrozenWormTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
        ...
        {
            scale = 0.9f;   //this make the NPC Health Bar biger
            return null;
        }
	...
	public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
            ...
            return spawnInfo.spawnTileType == TileID.IceBlock && spawnInfo.player.ZoneSnow && spawnInfo.player.ZoneRockLayerHeight ? .1f : 0f;
		}

	public override void NPCLoot()
	{
	{
                Item.NewItem(npc.getRect(), mod.ItemType("IceBone"));
	}
	}
