...
{
    public class IceWormHead : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            npc.lifeMax = 3000;        //this is the npc health
            ...
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("IceWormBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("IceWormTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
        ...
        {
            scale = 1.1f;   //this make the NPC Health Bar biger
            return null;
        }

        public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
            return spawnInfo.spawnTileType == TileID.IceBlock && spawnInfo.player.ZoneSnow && spawnInfo.player.ZoneRockLayerHeight && Main.hardMode ? .1f : 0f;
		}

	public override void NPCLoot()
	{
	{
                Item.NewItem(npc.getRect(), mod.ItemType("FrozenBlood"));
	}
	}
